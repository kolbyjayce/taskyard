...
export function registerTaskTools(
  ...
  server.tool(
    "read_task",
    "Read the full contents of a task file including its agent log.",
    {
      project: z.string(),
  ...
  server.tool(
    "claim_task",
    "Atomically claim a task. Fails if another agent already holds it.",
    {
      project: z.string(),
      task_id: z.string(),
  ...
  server.tool(
    "heartbeat",
    "Update the heartbeat timestamp for a claimed task. Call every 5 minutes.",
    {
      project: z.string(),
      task_id: z.string(),
      agent_id: z.string(),
  ...
  server.tool(
    "append_log",
    "Append a timestamped entry to the task's agent log. Updates last_progress_at.",
    {
      project: z.string(),
      task_id: z.string(),
      agent_id: z.string(),
  ...
  server.tool(
    "write_checkpoint",
    "Write a HANDOFF.md for this task. Call when approaching context limit before release_task.",
    {
      project: z.string(),
      task_id: z.string(),
      agent_id: z.string(),
  ...
  server.tool(
    "release_task",
    "Gracefully release a task back to the pool (e.g. context limit reached). Write a checkpoint first.",
    {
      project: z.string(),
      task_id: z.string(),
      agent_id: z.string(),
  ...
  server.tool(
    "complete_task",
    "Mark a task as done. Releases the lock and records a summary.",
    {
      project: z.string(),
      task_id: z.string(),
      agent_id: z.string(),
  ...
  server.tool(
    "get_status",
    "Get a summary of the current board — counts by status, stalled tasks, recent activity.",
    {
      project: z.string(),
    },
    async ({ project }) => {
      ...
      const stalled = tasks.filter(t => {
        ...
        if (!t.last_progress_at) return false;
